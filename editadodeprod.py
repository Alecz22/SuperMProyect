...
from bll.productos import actualizarp, obtener_idp
from datetime import date

class Editar_prod(tk.Toplevel):
    def __init__(self, master=None, prod_id=None):
        super().__init__(master)
        self.master=master
        self.prod_id=prod_id
        self.title("Editado de Producto")
        width=434
        height=411
        ...
        if prod_id is not None:
            producto = obtener_idp(prod_id)
            if producto is None:
               tkMsgBox.showerror(self.master.title(), "Se produjo un error al obtener los datos del producto, reintente nuevamente")
               self.destroy()
            else:
                GLineEdit_411.insert(0, producto[2])
                GLineEdit_540.insert(0, producto[1])
                GLineEdit_869.insert(0, producto[3])
                GLineEdit_217.insert(0, producto[4])
    ...
    def aceptar(self):
        try:  
            ...
            if self.prod_id is not None:
                ...
                actualizarp(self.prod_id,Producto, Cantidad,Categoria, Precio) 
